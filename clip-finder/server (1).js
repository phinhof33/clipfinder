...
app.post('/api/analyze', async (req, res) => {
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      console.log(`[subtitle check] ${candidate.title}: ${cues ? cues.length + ' cues found' : 'no subtitle file found'}`);
      if (!cues) continue;
      const match = findBestTimestamp(transcript.text, cues);
      console.log(`[subtitle check] ${candidate.title}: best match score = ${match ? match.confidence : 'null'}`);
